...
function StatusBadge({ status }: { status: string }) {
  return (
    ...
    >
      {proposalStatusLabel(status as ProposalWithItems["status"])}
    </span>
  );
}
  ...
  return (
    <>
      <Button asChild size="icon" variant="outline">
        <Link aria-label={`Edit ${row.title}`} href={href(`/proposals/${row.id}`)}>
    ...
    <ResponsiveDataList
      cards={rows.map((row) => (
        <MobileDataCard
          ...
          meta={
            <>
              <span>{formatProposalMoney(row.total_amount)}</span>
              <span>
                {new Date(`${row.issued_at}T12:00:00`).toLocaleDateString()}
              </span>
            </>
          }
          subtitle={row.proposal_number}
          title={
            <Link
