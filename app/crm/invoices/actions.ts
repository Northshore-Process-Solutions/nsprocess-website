...
function revalidateInvoiceTargets(input: {
  ...
  revalidatePath("/crm/statements");
  revalidatePath("/crm/agreements");
  revalidatePath("/crm");
  if (input.id) {
    revalidatePath(`/crm/invoices/${input.id}`);
    revalidatePath(`/crm/invoices/${input.id}/pdf`);
  }
  if (input.organizationId) {
    revalidatePath(`/crm/organizations/${input.organizationId}`);
  }
  if (input.agreementId) {
    revalidatePath(`/crm/agreements/${input.agreementId}`);
  }
}
